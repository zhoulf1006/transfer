...
**sent-images**: userData 下存发送截图副本的**内部目录**,不暴露给用户;设置里只展示**接收文件夹**。

**接收文件夹**: 收到的文件落盘的位置,用户可在设置里选定;**未选定时用系统下载目录**(配置里存 null 而非默认路径的字符串——存了字符串,系统下载目录一变就留下过期值)。目录不可用时**自动退回默认并且不保留原路径**,靠一个持久化的布尔标记告知用户(见 ADR-0022)。展示给用户的路径**解过符号链接**、落盘用的**不解**:沙盒下容器里的 Downloads 是指向真实 `~/Downloads` 的符号链接,两者落到同一个地方,但展示容器路径会让人在 Finder 里找不到。
...
- **进度不落库**:progress 只走 IPC 实时推,DB 只存最终 status;节流状态在**任意终态**统一清理,不依赖 100% 帧。
  推论边界:**终态结果可以落库**(如传输总耗时),不落的是过程量。两者的区别是"会不会持续变化",不是"跟不跟传输有关"。
- **表结构改动一律改列声明,不手写 ALTER**(ADR-0021):建表与补列都从同一份声明派生,"加了列却忘了补"因此在结构上不可能发生。两条配套硬约束:
  **①新增列必须可空**(不写 NOT NULL、不给默认值)——有数据的老库补不上不可空的列,而用户降级回旧版时旧版的 INSERT 不带新列;
  **②改动前的建表语句要作为快照追加进迁移测试**,漏了不报错,只是从那个版本升级的路径失去覆盖。
  ⚠️ 别指望 SQLite 替你把关:实测"NOT NULL 且无默认值"**只在有数据的表上才被拒绝**,空表照样能加 —— 开发时用新建库永远碰不到,只有老用户升级才炸。
- **传输耗时的起点只在内存,不落库**:从"开始"到"终态"必在同一次运行内(重启则传输中断,那条消息会被标 expired,根本走不到终态)。起点按方向取——发送方是消息创建时刻(点发送即开始,含发送前算摘要那段),接收方是转 accepted 那一刻(此前是用户自己在犹豫,不算传输)。**拿不到起点时耗时为 null,界面据此整个不显示**,不得退化成 0。
