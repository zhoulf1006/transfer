...
/**
 * zh 是键集合的唯一真源:UIKey 从它推导,en 必须实现同一组键(Record<UIKey, string>),
 * 缺键即编译报错。与 app 侧 src/shared/i18n/dict.ts 同款做法。
 *
 * 为什么非得靠类型卡死:t() 在当前语言缺键时会回落到 defaultLocale(= zh),
 * 于是英文页会静默混进一句中文——astro build 不做类型检查、CI 也不跑 check,
 * 漏译能一路上线且无任何告警。键集合对齐另有 ui.test.ts 兜底。
 */
const zh = {
  'site.name': 'Transfer',
  'nav.features': '功能',
 ...
 * 取当前 locale 的翻译函数。
 *
 * key 收 string 而不收 UIKey:调用点有 `t(\`features.${f.key}.title\`)` 与 `t(item.labelKey)`
 * 这类动态拼键,收窄成联合会把它们全挡在门外。故键的正确性靠 ui 的类型 + ui.test.ts 保证,
 * 不靠调用点签名;查表的 `as UIKey` 只是绕开索引签名,`??` 兜底仍然生效。
 */
export function useTranslations(locale: Locale) {
  return function t(key: string): string {
    return ui[locale][key as UIKey] ?? ui[defaultLocale][key as UIKey] ?? key
  }
}
