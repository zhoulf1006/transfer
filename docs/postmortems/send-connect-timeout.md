...
## 后续(2026-08-21,只追加不改写上文)

上文提到的 `T_UPLOAD_MS`(5min **总时长**)**已不存在**,被 `T_UPLOAD_IDLE_MS`(45s **空闲**)取代,见 ADR-0020。
grep 旧名字找不到东西时以此为准。

本文那条"回归红线"(握手后必须 `socket.setTimeout(0)`,否则大文件上传的空闲期被 10s 建连超时误杀)**仍然成立且更要紧了**:
当时只有建连超时会误杀空闲期,后来发现**会话级也有同一类错误**——接收方的 30s 空闲计时器在传输体流动期间从不刷新,
任何耗时超过 30 秒的单文件传输都会被判空闲清掉。同一个观念错误(把"传了多久"当失败判据)在两个层面各犯了一次,
第二次一直没被发现,因为它的故障是静默的(文件照收、upload 照回 200,只是不上报完成)。
